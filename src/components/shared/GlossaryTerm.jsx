...
const glossaryEntryCache = new Map()
...
async function loadGlossaryEntry(term) {
  ...
  if (!glossaryModulePromise) {
    glossaryModulePromise = import('../../lib/glossary.js')
  }

  try {
    const glossaryModule = await glossaryModulePromise
    const entry = glossaryModule.getGlossaryEntry(term) ?? null
    glossaryEntryCache.set(cacheKey, entry)
    return entry
  } catch {
    glossaryEntryCache.set(cacheKey, null)
    return null
  }
}

function GlossaryTermInner({
  term,
  children = null,
  className = '',
  tooltipAlign = 'left',
  ...
  const [entry, setEntry] = useState(() =>
    glossaryEntryCache.has(cacheKey) ? glossaryEntryCache.get(cacheKey) : null,
  )
  const [hasLoadedEntry, setHasLoadedEntry] = useState(() =>
    glossaryEntryCache.has(cacheKey),
  )
  const requestEntry = useCallback(() => {
    if (!cacheKey || hasLoadedEntry) {
      return
    }

    setHasLoadedEntry(true)
    void loadGlossaryEntry(term).then((nextEntry) => {
      if (isMountedRef.current) {
        setEntry(nextEntry)
      }
    })
  }, [cacheKey, hasLoadedEntry, term])
  ...
  if (!cacheKey || (hasLoadedEntry && !entry)) {
  ...
  return (
    <span
      className={`group relative inline-flex items-center gap-1 align-middle ${className}`}
      onFocus={requestEntry}
      onMouseEnter={requestEntry}
    >
      <span>{label}</span>
      ...
      <span
        ...
        aria-label={entry ? `${entry.label}: ${entry.short}` : 'Load glossary note'}
      >
        i
      </span>
...
function GlossaryTerm(props) {
  return <GlossaryTermInner key={getCacheKey(props.term)} {...props} />
}
